/**********************************************************************
 *
 *   Component generated by Quest
 *
 *   WARNING: By editing this component by hand, you will lose the ability to regenerate the code without conflicts.
 *   To preserve that ability, always export from Quest to regenerate this file.
 *   To setup element attributes and CSS properties, use the Quest web app
 *   Code Logic goes in the hook associated with this component
 *
 *   For help and further details refer to: https://www.quest.ai/docs
 *
 *
 **********************************************************************/

import React from 'react'
import { styled } from '@mui/material/styles'
import ShopNavbar1 from 'components/ShopNavbar1/ShopNavbar1'
import ProductPage1 from 'components/ProductPage1/ProductPage1'
import ShoppingFooter1 from 'components/ShoppingFooter1/ShoppingFooter1'
import useLayoutProductPage1 from 'components/LayoutProductPage1/useLayoutProductPage1'

const ScreenDesktop = styled('div')({
  backgroundColor: `rgba(255, 255, 255, 1)`,
  display: `flex`,
  position: `relative`,
  isolation: `isolate`,
  flexDirection: `column`,
  justifyContent: `flex-start`,
  alignItems: `flex-start`,
  padding: `0px`,
  boxSizing: `border-box`,
  overflow: `hidden`,
  height: 'auto',
  width: '100vw',
})

const ShopNavbar11 = styled(ShopNavbar1, {
  shouldForwardProp: (prop) => !['data'].includes(prop.toString()),
})(({ theme, data }) => ({
  alignSelf: `stretch`,
  height:
    data.currentVariant === 'ScreenTablet'
      ? `50px`
      : data.currentVariant === 'ScreenMobile'
      ? 'unset'
      : `60px`,
  margin: `0px`,
}))

const ProductPage11 = styled(ProductPage1)(({ theme }) => ({
  alignSelf: `stretch`,
  margin: `10px 0px 0px 0px`,
}))

const ShoppingFooter11 = styled(ShoppingFooter1, {
  shouldForwardProp: (prop) => !['data'].includes(prop.toString()),
})(({ theme, data }) => ({
  width:
    data.currentVariant === 'ScreenTablet'
      ? `768px`
      : data.currentVariant === 'ScreenMobile'
      ? `375px`
      : `1440px`,
  margin: `10px 0px 0px 0px`,
}))

function LayoutProductPage1({ id, ...props }) {
  const { data } = useLayoutProductPage1()
  return (
    <ScreenDesktop className={props.className}>
      <ShopNavbar11 data={data} cart={props.cart} />
      <ProductPage11 id={id} handleAddToCart={props.handleAddToCart} />
      <ShoppingFooter11 data={data} />
    </ScreenDesktop>
  )
}

export default LayoutProductPage1
